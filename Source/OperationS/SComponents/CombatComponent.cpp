...
#include "Operations/Weapon/Projectile.h"
...
{
	if (EquippedWeapon->EquipSound && Character)
	{
...
void UCombatComponent::SetHUDCrosshairs(float DeltaTime)
{
	...
	if (Controller)
	{
		...
		if (HUD)
		{
			...
			{
				CrosshairInAirFactor = FMath::FInterpTo(CrosshairInAirFactor, 2.25f, DeltaTime, 20.f);
			}
			else
			{
			...
			{
				CrosshairSprintingFactor = FMath::FInterpTo(CrosshairSprintingFactor, 2.25f, DeltaTime, 20.f);
			}
			else
			{
				CrosshairSprintingFactor = FMath::FInterpTo(CrosshairSprintingFactor, 0.f, DeltaTime, 30.f);
			...
			HUDPackage.CrosshairSpread = 0.5f + CrosshairVelocityFactor + CrosshairInAirFactor + CrosshairSprintingFactor - CrosshairAimFactor + CrosshairShootingFactor;
...
{
	if (CombatState == ECombatState::ECS_Unoccupied && EquippedWeapon && EquippedWeapon->Ammo < EquippedWeapon->MagCapacity)
	{
		ServerReload();
	}
}
